...
class TargetListPlots:
    def __init__(self):
        self.gene_file = os.path.join(os.getcwd(), "Supplemental_table_1.xlsx")

    def buildPlots(self):
        df = pd.read_excel(self.gene_file)
        ...
        ax6.text(x=1, y=143, s=str(grouped.loc["2", "0"]), fontsize=10, ha='center', va='center')
        ax6.text(x=1, y=327, s=str(grouped.loc["2", "1"]), fontsize=10, ha='center', va='center')
        ax6.text(x=2, y=135, s=str(grouped.loc["3", "0"]), fontsize=10, ha='center', va='center')
        ax6.text(x=2, y=309, s=str(grouped.loc["3", "1"]), fontsize=10, ha='center', va='center')
        ax6.text(x=3, y=20, s=str(grouped.loc["4", "0"]), fontsize=10, ha='center', va='center')
        ax6.text(x=3, y=50, s=str(grouped.loc["4", "1"]), fontsize=10, ha='center', va='bottom')
        ax6.text(x=4, y=20, s=str(grouped.loc["5", "0"]), fontsize=10, ha='center', va='center')
        ax6.text(x=4, y=50, s=str(grouped.loc["5", "1"]), fontsize=10, ha='center', va='bottom')
        ax6.text(x=5, y=-2, s=str(grouped.loc["-", "1"]), fontsize=10, ha='center', va='bottom')
        ...
        df['nTMR'] = df['nTMR'].fillna(-1).astype(int)
        transmembrane = df.groupby(['nTMR']).size()
        ...
        print("Number of entries with no signal peptides:", blank_signal_peptides)
        ...
        print("Saving panel as target-list-plots.svg")
        plt.savefig("target-list-plots.svg", format='svg', bbox_inches='tight')
        print("Done")
