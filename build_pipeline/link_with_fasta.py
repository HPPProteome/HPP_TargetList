...
#Creates dictionary to store data from FASTA file
genes_dict = {}
symbols_dict = {}
ensg_dict = {}

#Creates 3 dictionaries from FASTA file
...
#Matches GENCODE sequence information with ENSP number
gene_file['sequence'] = ''
...
#Matches based on gene symbol and takes the sequence information from the highest CDS length
#There can be muiltple data points for the same gene_symbol becuase it is not unique like ENSP numbers are 
for index, row in gene_file.iterrows():
	if row['CDS'] == "nan":
...
#Takes highest CDS length for any genes that don't have a UniProt Counterpart
#Link made with ENSG number
for index, row in gene_file.iterrows():
...
#Forces columns to integers so that the difference and length can be computed
...
columns_to_export = [
    ...
    'EC Number', 'Num Transmembrane Regions', 'Signal Peptide', 'PeptideAtlas Category', 'Observed', 'Distinct', 'Uniquely Mapping']
...
print("Main table used for final script: sequence_table.xlsx")
